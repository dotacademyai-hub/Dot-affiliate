...
import { Moon, Sun, ArrowLeft, ArrowRight, CheckCircle, Loader2 } from "lucide-react";
    ...
    <div className="flex flex-col gap-2">
      {options.map((opt) => (
        <label key={opt.value} className={`flex items-center gap-3 p-3 rounded-xl border cursor-pointer transition-all ${value === opt.value ? "border-primary bg-primary/10 text-foreground" : "border-border hover:border-primary/40"}`}>
          <div className={`w-4 h-4 rounded-full border-2 flex items-center justify-center ${value === opt.value ? "border-primary" : "border-muted-foreground/40"}`}>
        ...
        <div className="w-full max-w-md">
          ...
          <div className="flex rounded-xl border border-border bg-card p-1 mb-8">
            <button
              ...
              className={`flex-1 py-2.5 rounded-lg text-sm font-semibold transition-all ${mode === "login" ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:text-foreground"}`}
            ...
            <button
              ...
              className={`flex-1 py-2.5 rounded-lg text-sm font-semibold transition-all ${mode === "signup" ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:text-foreground"}`}
            ...
            <div data-testid="login-form">
              ...
              <form onSubmit={onLogin} className="space-y-4">
                <div>
                  ...
                  <Input id="login-email" type="email" placeholder="you@example.com" {...loginForm.register("email")} className="mt-1" data-testid="input-login-email" />
                ...
                <div>
                  ...
                  <Input id="login-password" type="password" placeholder="••••••••" {...loginForm.register("password")} className="mt-1" data-testid="input-login-password" />
              ...
              <p className="text-center text-sm text-muted-foreground mt-6">
                Not a partner yet?{" "}
                <button onClick={() => switchMode("signup")} className="text-primary font-semibold hover:underline" data-testid="button-switch-to-signup">
                  Apply now
                </button>
              </p>
            </div>
            ...
            <div data-testid="signup-form">
              <div className="mb-6">
                ...
                <p className="text-xs text-muted-foreground leading-relaxed">
                  We're onboarding a limited number of influencers to help drive attendance. If selected, you'll receive a unique tracking link, incentives, and direct access to our team. Only serious applicants should proceed.
                </p>
              </div>
              ...
              {step === 1 && (
                <div className="space-y-4" data-testid="signup-step-1">
                  ...
                  <div>
                    ...
                    <Input id="email" type="email" placeholder="you@example.com" {...step1Form.register("email")} className="mt-1" data-testid="input-email" />
                  ...
                  <div>
                    ...
                    <Input id="password" type="password" placeholder="Min. 8 characters" {...step1Form.register("password")} className="mt-1" data-testid="input-password" />
                    {step1Form.formState.errors.password && <p className="text-destructive text-xs mt-1">{step1Form.formState.errors.password.message}</p>}
                  </div>
                  <div>
              ...
              {step === 2 && (
                <div className="space-y-5" data-testid="signup-step-2">
                  <div>
                    ...
                    <RadioGroup
                      options={PLATFORMS}
                      value={step2Form.watch("primaryPlatform") ?? ""}
                      onChange={(v) => step2Form.setValue("primaryPlatform", v as never, { shouldValidate: true })}
                ...
                <div className="space-y-5" data-testid="signup-step-3">
                  <div>
                    ...
                    <RadioGroup
                      ...
                      value={step3Form.watch("hasPromotedBefore") ?? ""}
                      onChange={(v) => step3Form.setValue("hasPromotedBefore", v as never, { shouldValidate: true })}
                      name="hasPromotedBefore"
                    />
                  ...
                  <div>
                    ...
                    <RadioGroup
                      options={[{ value: "0-50", label: "0–50" }, { value: "50-100", label: "50–100" }, { value: "100+", label: "100+" }]}
                      value={step3Form.watch("ticketsSellEstimate") ?? ""}
                      onChange={(v) => step3Form.setValue("ticketsSellEstimate", v as never, { shouldValidate: true })}
                      name="ticketsSellEstimate"
                    />
                  ...
                  <div>
                    ...
                    <RadioGroup
                      options={[{ value: "true", label: "YES" }, { value: "false", label: "NO" }]}
                      value={step3Form.watch("willingToPromote") ?? ""}
                      onChange={(v) => step3Form.setValue("willingToPromote", v as never, { shouldValidate: true })}
                      name="willingToPromote"
                    />
              ...
              {step === 4 && (
                <div className="space-y-4" data-testid="signup-step-4">
                  <div>
                    ...
                    <Textarea
                      id="why"
                      placeholder="Tell us about yourself, your audience, and why you're the right fit for FEARLESS WEEK 2.0..."
                      rows={6}
                      {...step4Form.register("whySelectYou")}
              ...
              <p className="text-center text-sm text-muted-foreground mt-6">
                Already a partner?{" "}
                <button onClick={() => switchMode("login")} className="text-primary font-semibold hover:underline" data-testid="button-switch-to-login">
                  Sign in
                </button>
              </p>
            </div>
